...
const sheetVariants = cva(
  "fixed z-50 gap-5 bg-background p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 font-sans",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-full opacity-70 ring-offset-background transition-all hover:opacity-100 hover:bg-muted focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none p-1.5">
        <X className="h-4 w-4" />
...
const SheetHeader = ({
  ...
  <div
    className={cn(
      "flex flex-col space-y-2 text-center sm:text-left bg-[#D3E4FD] p-4 -m-6 mb-4 rounded-t",
      className
    )}
    {...props}
  />
